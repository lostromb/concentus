...
{
    internal static unsafe class Rate
    {
        ...
        internal static readonly byte* LOG2_FRAC_TABLE = AllocateGlobalArray<byte>(24, new byte[] {
           0,
           8,13,
          16,19,21,23,
        ...
        internal static unsafe int get_pulses(int i)
        {
        ...
        internal static unsafe int interp_bits2pulses(in OpusCustomMode* m, int start, int end, int skip_start,
              in int* bits1, in int* bits2, in int* thresh, in int* cap, int total, int* _balance,
              ...
              int* ebits, int* fine_priority, int C, int LM, ec_ctx* ec, int encode, int prev, int signalBandwidth)
        {
            ...
            for (j = end; j-- > start;)
            {
                int tmp = bits1[j] + ((int)lo * bits2[j] >> ALLOC_STEPS);
                if (tmp < thresh[j] && done == 0)
                {
                    if (tmp >= alloc_floor)
                        tmp = alloc_floor;
            ...
            if (intensity_rsv > 0)
            {
                if (encode != 0)
                {
                    ...
                    *intensity = (int)(start + ec_dec_uint(ec, (uint)(codedBands + 1 - start)));
            }
            else
                *intensity = 0;
            if (*intensity <= start)
            ...
            for (j = start; j < codedBands; j++)
            {
                ...
                if (N > 1)
                {
                    ...
                    fine_priority[j] = BOOL2INT(ebits[j] * (den << BITRES) >= bits[j] + offset);
                ...
                if (excess > 0)
                {
                    ...
                    fine_priority[j] = BOOL2INT(extra_bits >= excess - balance);
                    excess -= extra_bits;
                }
                balance = excess;
            ...
            *_balance = balance;
            ...
            for (; j < end; j++)
            {
                ...
                bits[j] = 0;
                fine_priority[j] = BOOL2INT(ebits[j] < 1);
            }

            return codedBands;
        }

        internal static unsafe int clt_compute_allocation(
            in OpusCustomMode* m, int start, int end, in int* offsets, in int* cap, int alloc_trim, int* intensity, int* dual_stereo,
              int total, int* balance, int* pulses, int* ebits, int* fine_priority, int C, int LM, ec_ctx* ec, int encode, int prev, int signalBandwidth)
        {
            int lo, hi, len, j;
            int codedBands;
            int skip_start;
            ...
            int[] bits1_data = new int[len];
            int[] bits2_data = new int[len];
            int[] thresh_data = new int[len];
            int[] trim_offset_data = new int[len];
            fixed (int* bits1 = bits1_data)
            fixed (int* bits2 = bits2_data)
            fixed (int* thresh = thresh_data)
            fixed (int* trim_offset = trim_offset_data)
            {
                for (j = start; j < end; j++)
                {
