...
namespace ParityTest
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            int[] Applications = new int[] { OpusDefines.OPUS_APPLICATION_AUDIO, OpusDefines.OPUS_APPLICATION_VOIP, OpusDefines.OPUS_APPLICATION_RESTRICTED_LOWDELAY };
            ...
            int[] ForceModes = new int[] { OpusDefines.OPUS_AUTO, OpusPrivate.MODE_CELT_ONLY, OpusPrivate.MODE_SILK_ONLY };
                                            ...
                                            for (int dtx_idx = 0; dtx_idx < DTXModes.Length; dtx_idx++)
                                            {
                                                for (int vbr_idx = 0; vbr_idx < VBRModes.Length; vbr_idx++)
                                                {
                                                    ...
                                                    if (newParams.Bitrate > 40 || newParams.FrameSize < 10)
                                                    {
                                                        ...
                                                        if (newParams.ForceMode == OpusPrivate.MODE_SILK_ONLY)
                                                        {
                                                            continue;
                                                        }
                                                    }
                                                    // Constrained VBR only applies to CELT
                                                    if (newParams.ForceMode == OpusPrivate.MODE_SILK_ONLY && newParams.ConstrainedVBR)
                                                    {
                                                        continue;
                                                    }
        ...
        {
            if (app == OpusDefines.OPUS_APPLICATION_AUDIO)
                return "Music   ";
            else if (app == OpusDefines.OPUS_APPLICATION_VOIP)
                return "Voip    ";
            return "LowDelay";
        }
        ...
        {
            if (mode == OpusPrivate.MODE_CELT_ONLY)
                return "CELT";
            if (mode == OpusPrivate.MODE_SILK_ONLY)
                return "SILK";
            return "    ";
        }
