...
{
    public static class pitch_analysis_core
    {
        ...
        /*************************************************************/
        public static int silk_pitch_analysis_core(                  /* O    Voicing estimate: 0 voiced, 1 unvoiced                      */
        ...
        )
        {
            ...
            Debug.Assert(Fs_kHz == 8 || Fs_kHz == 12 || Fs_kHz == 16);

            /* Check for valid complexity setting */
            Debug.Assert(complexity >= SilkConstants.SILK_PE_MIN_COMPLEX);
            Debug.Assert(complexity <= SilkConstants.SILK_PE_MAX_COMPLEX);

            Debug.Assert(search_thres1_Q16 >= 0 && search_thres1_Q16 <= (1 << 16));
            Debug.Assert(search_thres2_Q13 >= 0 && search_thres2_Q13 <= (1 << 13));
            ...
            ******************************************************************************/
            ...
            for (k = 0; k < nb_subfr >> 1; k++)
            {
                ...
                Debug.Assert(target_ptr.Offset >= frame_4kHz.Offset);
                Debug.Assert(target_ptr.Offset + SF_LENGTH_8KHZ <= frame_4kHz.Offset + frame_length_4kHz);

                basis_ptr = target_ptr.Point(0 - MIN_LAG_4KHZ);

                /* Check that we are within range of the array */
                Debug.Assert(basis_ptr.Offset >= frame_4kHz.Offset);
                Debug.Assert(basis_ptr.Offset + SF_LENGTH_8KHZ <= frame_4kHz.Offset + frame_length_4kHz);

                celt_pitch_xcorr.pitch_xcorr(target_ptr, target_ptr.Point(0 - MAX_LAG_4KHZ), xcorr32, SF_LENGTH_8KHZ, MAX_LAG_4KHZ - MIN_LAG_4KHZ + 1);
            ...
            Debug.Assert(3 * length_d_srch <= SilkConstants.PE_D_SRCH_LENGTH);
            Sort.silk_insertion_sort_decreasing_int16(C, d_srch, CSTRIDE_4KHZ, length_d_srch);

            /* Escape if correlation is very low already here */
            Cmax = (int)C[0];                                                    /* Q14 */
            if (Cmax < Inlines.SILK_FIX_CONST(0.2f, 14))
            {
            ...
            *********************************************************************************/
            ...
            for (k = 0; k < nb_subfr; k++)
            {
                ...
                /* Check that we are within range of the array */
                Debug.Assert(target_ptr.Offset >= frame_8kHz.Offset);
                Debug.Assert(target_ptr.Offset + SF_LENGTH_8KHZ <= frame_8kHz.Offset + frame_length_8kHz);
                ...
                for (j = 0; j < length_d_comp; j++)
                {
                    ...
                    /* Check that we are within range of the array */
                    Debug.Assert(basis_ptr.Offset >= frame_8kHz.Offset);
                    Debug.Assert(basis_ptr.Offset + SF_LENGTH_8KHZ <= frame_8kHz.Offset + frame_length_8kHz);
            ...
            Debug.Assert(search_thres2_Q13 == Inlines.silk_SAT16(search_thres2_Q13));
            ...
            for (k = 0; k < length_d_srch; k++)
            {
                ...
                Debug.Assert(lag_log2_Q7 == Inlines.silk_SAT16(lag_log2_Q7));
                Debug.Assert(nb_subfr * Inlines.SILK_FIX_CONST(SilkConstants.PE_SHORTLAG_BIAS, 13) == Inlines.silk_SAT16(nb_subfr * Inlines.SILK_FIX_CONST(SilkConstants.PE_SHORTLAG_BIAS, 13)));
                CCmax_new_b = CCmax_new - Inlines.silk_RSHIFT(Inlines.silk_SMULBB(nb_subfr * Inlines.SILK_FIX_CONST(SilkConstants.PE_SHORTLAG_BIAS, 13), lag_log2_Q7), 7); /* Q13 */

                /* Bias towards previous lag */
                Debug.Assert(nb_subfr * Inlines.SILK_FIX_CONST(SilkConstants.PE_PREVLAG_BIAS, 13) == Inlines.silk_SAT16(nb_subfr * Inlines.SILK_FIX_CONST(SilkConstants.PE_PREVLAG_BIAS, 13)));
                if (prevLag > 0)
                {
                    delta_lag_log2_sqr_Q7 = lag_log2_Q7 - prevLag_log2_Q7;
                    Debug.Assert(delta_lag_log2_sqr_Q7 == Inlines.silk_SAT16(delta_lag_log2_sqr_Q7));
                    delta_lag_log2_sqr_Q7 = Inlines.silk_RSHIFT(Inlines.silk_SMULBB(delta_lag_log2_sqr_Q7, delta_lag_log2_sqr_Q7), 7);
                    prev_lag_bias_Q13 = Inlines.silk_RSHIFT(Inlines.silk_SMULBB(nb_subfr * Inlines.SILK_FIX_CONST(SilkConstants.PE_PREVLAG_BIAS, 13), LTPCorr_Q15.Val), 15); /* Q13 */
                    prev_lag_bias_Q13 = Inlines.silk_DIV32(Inlines.silk_MUL(prev_lag_bias_Q13, delta_lag_log2_sqr_Q7), delta_lag_log2_sqr_Q7 + Inlines.SILK_FIX_CONST(0.5f, 7));
            ...
            Debug.Assert(LTPCorr_Q15.Val >= 0);
            ...
            if (Fs_kHz > 8)
            {
                ...
                Debug.Assert(lag == Inlines.silk_SAT16(lag));
                if (Fs_kHz == 12)
                {
                ...
                for (d = start_lag; d <= end_lag; d++)
                {
                    for (j = 0; j < nb_cbk_search; j++)
                    {
                        ...
                        if (cross_corr > 0)
                        {
                            ...
                            Debug.Assert(diff == Inlines.silk_SAT16(diff));
        ...
        )
        {
            ...
            if (nb_subfr == SilkConstants.PE_MAX_NB_SUBFR)
            {
                ...
                Debug.Assert(nb_subfr == SilkConstants.PE_MAX_NB_SUBFR >> 1);
            ...
            for (k = 0; k < nb_subfr; k++)
            {
                ...
                Debug.Assert(lag_high - lag_low + 1 <= SCRATCH_SIZE);
                celt_pitch_xcorr.pitch_xcorr(target_ptr, target_ptr.Point(0 - start_lag - lag_high), xcorr32, sf_length, lag_high - lag_low + 1);
                for (j = lag_low; j <= lag_high; j++)
                {
                    Debug.Assert(lag_counter < SCRATCH_SIZE);
                ...
                for (i = 0; i < nb_cbk_search; i++)
                {
                    ...
                    {
                        Debug.Assert(idx + j < SCRATCH_SIZE);
                        Debug.Assert(idx + j < lag_counter);
            ...
            for (k = 0; k < nb_subfr; k++)
            {
                ...
                for (i = 0; i < nb_cbk_search; i++)
                {
                    ...
                    for (j = 0; j < SilkConstants.PE_NB_STAGE3_LAGS; j++)
                    {
                        ...
                        Debug.Assert(Inlines.matrix_ptr(energies_st3, k, i, nb_cbk_search).Values[j] >= 0);
                    }
                }
                target_ptr = target_ptr.Point(sf_length);
            }
