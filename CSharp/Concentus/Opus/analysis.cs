...
namespace Concentus
{
    public static class analysis
    {
        ...
        //    if (Inlines.celt_isnan(output[0].r))
        //    {
        //        info.valid = 0;
        //        return;
        //    }
        ...
        //        mod1 = d2_angle - (float)Inlines.floor(0.5f + d2_angle);
        //        noisiness[i] = Inlines.ABS16(mod1);
        //        mod1 *= mod1;
        //        mod1 *= mod1;

        //        mod2 = d2_angle2 - (float)Inlines.floor(0.5f + d2_angle2);
        ...
        //            E += binE;
        //            tE += binE * tonality[i];
        //            nE += binE * 2.0f * (0.5f - noisiness[i]);
        //        }
        //        /* Check for extreme band energies that could cause NaNs later. */
        //        if (!(E < 1e9f) || Inlines.celt_isnan(E))
        //        {
        //            info.valid = 0;
        //            return;
        //        }
        ...
        //        frame_loudness += (float)Inlines.sqrt(E + 1e-10f);
        //        logE[b] = (float)Inlines.log(E + 1e-10f);
        ...
        //        {
        //            L1 += (float)Inlines.sqrt(tonal.E[i][b]);
        //            L2 += tonal.E[i][b];
        //        }

        //        stationarity = Inlines.MIN16(0.99f, L1 / (float)Inlines.sqrt(1e-15 + OpusConstants.NB_FRAMES * L2));
        ...
        //    if (tonal.count <= 2)
        //        bandwidth = 20;
        //    frame_loudness = 20 * (float)Inlines.log10(frame_loudness);
        ...
        //    }
        //    for (i = 0; i < 9; i++)
        //        features[11 + i] = (float)Inlines.sqrt(tonal.std[i]);
        ...
        //    frame_probs[0] = .01f + 1.21f * frame_probs[0] * frame_probs[0] - .23f * (float)Inlines.pow(frame_probs[0], 10);
        ...
        //        p0 *= (float)Inlines.pow(1 - frame_probs[0], beta);
        //        p1 *= (float)Inlines.pow(frame_probs[0], beta);
        ...
        //        psum = 1e-20f;
        //        /* Instantaneous probability of speech and music, with beta pre-applied. */
        //        speech0 = (float)Inlines.pow(1 - frame_probs[0], beta);
        //        music0 = (float)Inlines.pow(frame_probs[0], beta);
        //        if (tonal.count == 1)
        //        {
