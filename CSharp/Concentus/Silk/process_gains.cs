    ...
    public static class process_gains
    {
        ...
        )
        {
            ...
            InvMaxSqrVal_Q16 = Inlines.silk_DIV32_16(Inlines.silk_log2lin(
                Inlines.silk_SMULWB(Inlines.SILK_FIX_CONST(21 + 16 / 0.33f, 7) - psEnc.SNR_dB_Q7, Inlines.SILK_FIX_CONST(0.33f, 16))), psEnc.subfr_length);
            ...
            if (psEnc.indices.signalType == SilkConstants.TYPE_VOICED)
            {
                if (psEncCtrl.LTPredCodGain_Q7 + Inlines.silk_RSHIFT(psEnc.input_tilt_Q15, 8) > Inlines.SILK_FIX_CONST(1.0f, 7))
                {
            ...
            psEncCtrl.Lambda_Q10 = Inlines.SILK_FIX_CONST(TuningParameters.LAMBDA_OFFSET, 10)
                                  + Inlines.silk_SMULBB(Inlines.SILK_FIX_CONST(TuningParameters.LAMBDA_DELAYED_DECISIONS, 10), psEnc.nStatesDelayedDecision)
                                  + Inlines.silk_SMULWB(Inlines.SILK_FIX_CONST(TuningParameters.LAMBDA_SPEECH_ACT, 18), psEnc.speech_activity_Q8)
