...
func silk_pitch_analysis_core(frame []int16, pitch_out []int, lagIndex *BoxedValueShort, contourIndex *BoxedValueByte, LTPCorr_Q15 *BoxedValueInt, prevLag int, search_thres1_Q16 int, search_thres2_Q13 int, Fs_kHz int, complexity int, nb_subfr int) int {
	...
	OpusAssert(complexity >= SilkConstants.SILK_PE_MIN_COMPLEX)
	OpusAssert(complexity <= SilkConstants.SILK_PE_MAX_COMPLEX)

	OpusAssert(search_thres1_Q16 >= 0 && search_thres1_Q16 <= (1<<16))
	OpusAssert(search_thres2_Q13 >= 0 && search_thres2_Q13 <= (1<<13))
	...
	*/
	...
	if shift > 0 {
		...
		for i = 0; i < frame_length_4kHz; i++ {
			frame_4kHz[i] = silk_RSHIFT16(frame_4kHz[i], shift)
		}
	}
	...
	*/
	...
	for k = 0; k < nb_subfr>>1; k++ {
		...
		MatrixSetShort5(C, k, 0, CSTRIDE_4KHZ,
			int16(silk_DIV32_varQ(cross_corr, normalizer, 13+1)))
		/* Q13 */
		...
		for d = MIN_LAG_4KHZ + 1; d <= MAX_LAG_4KHZ; d++ {
			...
			normalizer = silk_ADD32(normalizer,
				silk_SMULBB(int(basis[basis_ptr]), int(basis[basis_ptr]))-
					silk_SMULBB(int(basis[basis_ptr+SF_LENGTH_8KHZ]), int(basis[basis_ptr+SF_LENGTH_8KHZ])))

			MatrixSetShort5(C, k, d-MIN_LAG_4KHZ, CSTRIDE_4KHZ,
				int16(silk_DIV32_varQ(cross_corr, normalizer, 13+1)))
			/* Q13 */
		}
	...
	if nb_subfr == SilkConstants.PE_MAX_NB_SUBFR {
		for i = MAX_LAG_4KHZ; i >= MIN_LAG_4KHZ; i-- {
			sum = int(MatrixGetShort(C, 0, i-MIN_LAG_4KHZ, CSTRIDE_4KHZ)) +
				int(MatrixGetShort(C, 1, i-MIN_LAG_4KHZ, CSTRIDE_4KHZ))
			/* Q14 */
		...
		/* Only short-lag bias */
		for i = MAX_LAG_4KHZ; i >= MIN_LAG_4KHZ; i-- {
			sum = silk_LSHIFT(int(C[i-MIN_LAG_4KHZ]), 1)
			/* Q14 */
			sum = silk_SMLAWB(sum, sum, silk_LSHIFT(-i, 4))
			/* Q14 */
			C[i-MIN_LAG_4KHZ] = int16(sum)
	...
	OpusAssert(3*length_d_srch <= SilkConstants.PE_D_SRCH_LENGTH)
	...
	if Cmax < int(math.Trunc((0.2)*float64(int64(1)<<(14))+0.5)) {
		MemSetLen(pitch_out, 0, nb_subfr)
		LTPCorr_Q15.Val = 0
		lagIndex.Val = 0
		contourIndex.Val = 0
		return 1
	...
	*/
	...
	if shift > 0 {
		...
		for i = 0; i < frame_length_8kHz; i++ {
			frame_8kHz[i] = silk_RSHIFT16(frame_8kHz[i], shift)
		}
	}
	...
	/**
	  ...
	  ********************************************************************************
	*/
	MemSetLen(C, 0, nb_subfr*CSTRIDE_8KHZ)

	target = frame_8kHz
	target_ptr = SilkConstants.PE_LTP_MEM_LENGTH_MS * 8
	for k = 0; k < nb_subfr; k++ {

		energy_target = silk_ADD32(silk_inner_prod(target, target_ptr, target, target_ptr, SF_LENGTH_8KHZ), 1)
		for j = 0; j < length_d_comp; j++ {
			...
			if cross_corr > 0 {
				...
				MatrixSetShort5(C, k, d-(MIN_LAG_8KHZ-2), CSTRIDE_8KHZ,
					int16(silk_DIV32_varQ(cross_corr, silk_ADD32(energy_target, energy_basis), 13+1)))
				/* Q13 */
			} else {
				MatrixSetShort5(C, k, d-(MIN_LAG_8KHZ-2), CSTRIDE_8KHZ, 0)
			}
		}
		target_ptr += SF_LENGTH_8KHZ
	}
	...
	OpusAssert(search_thres2_Q13 == silk_SAT16(search_thres2_Q13))
	...
	for k = 0; k < length_d_srch; k++ {
		...
		/* Q7 */
		OpusAssert(lag_log2_Q7 == silk_SAT16(lag_log2_Q7))
		OpusAssert(nb_subfr*int(float64(SilkConstants.PE_SHORTLAG_BIAS)*float64(int64(1)<<(13))+0.5) == silk_SAT16(nb_subfr*int(float64(SilkConstants.PE_SHORTLAG_BIAS)*float64(int64(1)<<(13))+0.5)))
		CCmax_new_b = CCmax_new - silk_RSHIFT(silk_SMULBB(nb_subfr*(int(float64(SilkConstants.PE_SHORTLAG_BIAS)*float64(int64(1)<<(13))+0.5)), lag_log2_Q7), 7)
		/* Q13 */

		/* Bias towards previous lag */
		OpusAssert(nb_subfr*int(float64(SilkConstants.PE_PREVLAG_BIAS)*float64(int64(1)<<(13))+0.5) == silk_SAT16(nb_subfr*int(float64(SilkConstants.PE_PREVLAG_BIAS)*float64(int64(1)<<(13))+0.5)))
		if prevLag > 0 {
			delta_lag_log2_sqr_Q7 = lag_log2_Q7 - prevLag_log2_Q7
			OpusAssert(delta_lag_log2_sqr_Q7 == silk_SAT16(delta_lag_log2_sqr_Q7))
			delta_lag_log2_sqr_Q7 = silk_RSHIFT(silk_SMULBB(delta_lag_log2_sqr_Q7, delta_lag_log2_sqr_Q7), 7)
			prev_lag_bias_Q13 = silk_RSHIFT(silk_SMULBB(nb_subfr*(int(float64(SilkConstants.PE_PREVLAG_BIAS)*float64(int64(1)<<(13))+0.5)), LTPCorr_Q15.Val), 15)
			/* Q13 */
			prev_lag_bias_Q13 = silk_DIV32(silk_MUL(prev_lag_bias_Q13, delta_lag_log2_sqr_Q7), delta_lag_log2_sqr_Q7+int(math.Trunc(0.5*float64(int64(1)<<(7))+0.5)))
	...
	LTPCorr_Q15.Val = silk_LSHIFT(silk_DIV32_16(CCmax, nb_subfr), 2)
	OpusAssert(LTPCorr_Q15.Val >= 0)
	...
	if Fs_kHz > 8 {
		...
		if shift > 0 {
			...
			for i = 0; i < frame_length; i++ {
				scratch_mem[i] = silk_RSHIFT16(frame[i], shift)
			}
		...
		lag_counter = 0
		OpusAssert(lag == silk_SAT16(lag))
		contour_bias_Q15 = silk_DIV32_16((int(float64(SilkConstants.PE_FLATCONTOUR_BIAS)*float64(int64(1)<<(15)) + 0.5)), lag)
			...
			for j = 0; j < nb_cbk_search; j++ {
				...
				for k = 0; k < nb_subfr; k++ {
					cross_corr = silk_ADD32(cross_corr,
						MatrixGetVals(cross_corr_st3, k, j,
							nb_cbk_search).Values[lag_counter])
					energy = silk_ADD32(energy,
						MatrixGetVals(energies_st3, k, j,
							nb_cbk_search).Values[lag_counter])
					OpusAssert(energy >= 0)
				}
				if cross_corr > 0 {
					...
					diff = math.MaxInt16 - silk_MUL(contour_bias_Q15, j)
					/* Q15 */
					OpusAssert(diff == silk_SAT16(diff))
		...
		for k = 0; k < nb_subfr; k++ {
			...
			pitch_out[k] = silk_LIMIT(pitch_out[k], min_lag, SilkConstants.PE_MAX_LAG_MS*Fs_kHz)
		}
...
func silk_P_Ana_calc_energy_st3(energies_st3 []*silk_pe_stage3_vals, frame []int16, start_lag int, sf_length int, nb_subfr int, complexity int) {
	...
	for k = 0; k < nb_subfr; k++ {
		...
		for i = 1; i < lag_diff; i++ {
			...
			energy = silk_ADD_SAT32(energy, int(frame[basis_ptr-i])*int(frame[basis_ptr-i]))
			scratch_mem[lag_counter] = energy
			lag_counter++
		}
