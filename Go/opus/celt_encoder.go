...
func (this *CeltEncoder) run_prefilter(input [][]int, prefilter_mem [][]int, CC int, N int, prefilter_tapset int, pitch *BoxedValueInt, gain *BoxedValueInt, qgain *BoxedValueInt, enabled int, nbAvailableBytes int) int {
	...
	if enabled != 0 {
		...
		gain1 = int(MULT16_16_Q15(int16(math.Trunc(0.5+0.7*float64(int(1<<15)))), int16(gain1)))
		if this.loss_rate > 2 {
			gain1 = HALF32(gain1)
		}
		if this.loss_rate > 4 {
	...
	pf_threshold = MAX16(pf_threshold, int16(math.Trunc(0.5+0.2*(1<<15))))
	...
	if gain1 < int(pf_threshold) {
		...
		if ABS32(gain1-this.prefilter_gain) < int(math.Trunc(0.5+0.1*float64(1<<15))) {
...
func (this *CeltEncoder) celt_encode_with_ec(pcm []int16, pcm_ptr int, frame_size int, compressed []byte, compressed_ptr int, nbCompressedBytes int, enc *EntropyCoder) int {
	...
	if this.lfe != 0 {
		for i = 2; i < end; i++ {
			bandE[0][i] = IMIN(bandE[0][i], MULT16_32_Q15Int(int(math.Trunc(0.5+(1e-4)*float64((1)<<(15)))), bandE[0][0]))
	...
	if this.lfe == 0 {
		...
		this.spec_avg += (MULT16_16_Q15Int(int(math.Trunc(0.5+(.02)*float64((1)<<(15)))), temporal_vbr))
	}
