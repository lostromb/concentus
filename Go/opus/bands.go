...
func anti_collapse(m *CeltMode, X_ [][]int, collapse_masks []int16, LM int, C int, size int, start int, end int, logE []int, prev1logE []int, prev2logE []int, pulses []int, seed int) {
	...
	for i = start; i < end; i++ {
		...
		depth = celt_udiv(1+pulses[i], int(m.eBands[i+1]-m.eBands[i])) >> LM

		thresh32 = SHR32(celt_exp2(int(0-SHL16(int16(depth), 10-BITRES))), 1)
		thresh = (MULT16_32_Q15(int16(math.Trunc(0.5+(0.5)*float64((int32(1))<<(15)))), MIN32(32767, thresh32)))
		{
			var t int
			t = N0 << LM
		...
		for {
			...
			Ediff = EXTEND32(int16(logE[c*m.nbEBands+i])) - EXTEND32(MIN16(int16(prev1), int16(prev2)))
			Ediff = MAX32(0, Ediff)

			if Ediff < 16384 {
				r32 := SHR32(celt_exp2(int(0-EXTRACT16(Ediff))), 1)
				r = (2 * MIN16Int(16383, (r32)))
			} else {
				r = 0
			}
			if LM == 3 {
				r = MULT16_16_Q14Int(23170, MIN32(23169, r)) // opus bug: was MIN32
			}
...
func stereo_split(X []int, X_ptr int, Y []int, Y_ptr int, N int) {
	for j := 0; j < N; j++ {
		l := MULT16_16(int(math.Trunc(0.5+(.70710678)*((1)<<(15)))), X[X_ptr+j])
		r := MULT16_16(int(math.Trunc(0.5+(.70710678)*((1)<<(15)))), Y[Y_ptr+j])
...
func haar1(X []int, X_ptr int, N0 int, stride int) {
	...
	for i = 0; i < stride; i++ {
		for j = 0; j < N0; j++ {
			...
			var tmp1, tmp2 int
			tmp1 = MULT16_16(int(int16(math.Trunc(0.5+(0.70710678)*float64(int32(1)<<(15))))), X[tmpidx])
			tmp2 = MULT16_16(int(int16(math.Trunc(0.5+(0.70710678)*float64(int32(1)<<(15))))), X[tmpidx+stride])
...
func haar1ZeroOffset(X []int, N0 int, stride int) {
	...
	for i = 0; i < stride; i++ {
		for j = 0; j < N0; j++ {
			...
			tmp1 := MULT16_16(int(math.Trunc(0.5+(.70710678)*((1)<<(15)))), X[tmpidx])
			tmp2 := MULT16_16(int(math.Trunc(0.5+(.70710678)*((1)<<(15)))), X[tmpidx+stride])
			X[tmpidx] = int(EXTRACT16(PSHR32(ADD32(tmp1, tmp2), 15)))
			X[tmpidx+stride] = int(EXTRACT16(PSHR32(SUB32(tmp1, tmp2), 15)))
		}
	}
...
func compute_theta(ctx *band_ctx, sctx *split_ctx, X []int, X_ptr int, Y []int, Y_ptr int, N int, b *BoxedValueInt, B int, B0 int, LM int, stereo int, fill *BoxedValueInt) {
	...
	if qn != 1 {
		...
		if stereo != 0 && N > 2 {
			...
			var x = itheta
			var x0 = qn / 2
			var ft = CapToUInt32(int64(p0*(x0+1) + x0))
...
func quant_partition(ctx *band_ctx, X []int, X_ptr int, N int, b int, B int, lowband []int, lowband_ptr int, LM int, gain int, fill int) int {
	...
	var side = 0
	var cm = 0
	var resynth = boolToInt(ctx.encode == 0)
	var Y = 0
	var encode int
	...
	if LM != -1 && b > int(cache[cache_ptr+int(cache[cache_ptr])])+12 && N > 2 {
		...
		boxed_b := &BoxedValueInt{b}
		boxed_fill := &BoxedValueInt{fill}
		...
		if mbits >= sbits {
			cm = quant_partition(ctx, X, X_ptr, N, mbits, B,
				...
				MULT16_16_P15Int(gain, mid), fill)
			...
			cm |= quant_partition(ctx, X, Y, N, sbits, B,
				...
				MULT16_16_P15Int(gain, side), fill>>B) << (B0 >> 1)
		} else {
			cm = quant_partition(ctx, X, Y, N, sbits, B,
				lowband, next_lowband2, LM,
				MULT16_16_P15Int(gain, side), fill>>B) << (B0 >> 1)
