...
func compute_vbr(mode *CeltMode, analysis *AnalysisInfo, base_target int, LM int, bitrate int, lastCodedBands int, C int, intensity int, constrained_vbr int, stereo_saving int, tot_boost int, tf_estimate int, pitch_change int, maxDepth int, variable_duration OpusFramesize, lfe int, has_surround_mask int, surround_masking int, temporal_vbr int) int {
	...
	if C == 2 {
		...
		max_frac = DIV32_16Int(MULT16_16(int(math.Trunc(0.5+(0.8)*((1)<<(15)))), coded_stereo_dof), coded_bins)

		//stereo_saving_val := MIN16(int16(stereo_saving), int16(0.5+1.0*float32(1<<8)))
		stereo_saving = MIN16Int(stereo_saving, int(math.Trunc(0.5+(1.0)*((1)<<(8)))))
		target -= int(MIN32(MULT16_32_Q15Int(max_frac, int(target)), SHR32(MULT16_16(int(stereo_saving)-int(math.Trunc(0.5+0.1*float64(1<<8))), int(coded_stereo_dof<<BITRES)), 8)))
	}
	...
	if (has_surround_mask == 0 || lfe != 0) && (constrained_vbr != 0 || bitrate < 64000) {
		...
		if constrained_vbr != 0 {
			rate_factor = MIN16Int(rate_factor, int(math.Trunc(0.5+0.67*float64(1<<15))))
		}
	...
	if has_surround_mask == 0 && tf_estimate < int(math.Trunc(0.5+0.2*float64(1<<14))) {
		amount := MULT16_16_Q15Int(int(math.Trunc(0.5+0.0000031*float64(1<<30))), int(IMAX(0, IMIN(32000, 96000-bitrate))))
...
func transient_analysis(input [][]int, len int, C int, tf_estimate *BoxedValueInt, tf_chan *BoxedValueInt) int {
	...
	for c := 0; c < C; c++ {
		...
		mem1 := 0
		for i := 0; i < len; i++ {
			x := SHR32(input[c][i], CeltConstants.SIG_SHIFT)
			y := ADD32(mem0, x)
		...
		norm := (len2 << (6 + 14)) / (CeltConstants.EPSILON + SHR32(mean, 1))
	...
	tf_estimate.Val = (celt_sqrt(MAX32(0, SHL32(MULT16_16(int(math.Trunc(0.5+(0.0069)*((1)<<(14)))), MAX16Int(163, tf_max)), 14)-int(math.Trunc(0.5+(0.139)*((1)<<(28)))))))
	return is_transient
}
...
func patch_transient_decision(newE [][]int, oldE [][]int, nbEBands int, start int, end int, C int) int {
	...
	if C == 1 {
		...
		for i := start + 1; i < end; i++ {
			spread_old[i] = MAX16Int(spread_old[i-1]-int(1.0*float32(int(1)<<CeltConstants.DB_SHIFT)), oldE[0][i])
		}
	} else {
		spread_old[start] = MAX16Int(oldE[0][start], oldE[1][start])
		for i := start + 1; i < end; i++ {
			spread_old[i] = MAX16Int(spread_old[i-1]-int(1.0*float32(int(1)<<CeltConstants.DB_SHIFT)), MAX16Int(oldE[0][i], oldE[1][i]))
		}
	}

	for i := end - 2; i >= start; i-- {
		spread_old[i] = MAX16Int(spread_old[i], spread_old[i+1]-int(1.0*float32(int(1)<<CeltConstants.DB_SHIFT)))
	}
...
func celt_preemphasis1(
	...
	if coef[1] == 0 && upsample == 1 && clip == 0 {
		for i = 0; i < N; i++ {
			...
			m = SHR32(MULT16_16(coef0, x), 15-CeltConstants.SIG_SHIFT)
		}
		mem.Val = m
		return
	}
...
func l1_metric(tmp []int, N int, LM int, bias int) int {
	L1 := 0
	for i := 0; i < N; i++ {
		L1 += EXTEND32Int(ABS32(tmp[i]))
	}
	L1 += MAC16_32_Q15Int(L1, int(LM*bias), int(L1))
	return L1
}
...
func tf_analysis(m *CeltMode, len int, isTransient int, tf_res []int, lambda int, X [][]int, N0 int, LM int, tf_sum *BoxedValueInt, tf_estimate int, tf_chan int) int {
	...
	bias = int(MULT16_16_Q14(int16(math.Trunc(0.5+0.04*(1<<15))), MAX16(int16(0)-int16(math.Trunc(0.5+0.25*(1<<14))), int16(math.Trunc(0.5+0.5*(1<<14)))-int16(tf_estimate))))
...
func stereo_analysis(m *CeltMode, X [][]int, LM int) int {
	...
	sumMS = MULT16_32_Q15Int(int(math.Trunc(0.707107*32767.5)), sumMS)
	thetas = 13
	if LM <= 1 {
		thetas -= 8
	}
...
func median_of_5(x []int, x_ptr int) int {
	...
	if t2 > t1 {
		if t1 < t3 {
			return MIN16Int(t2, t3)
		} else {
			return MIN16Int(t4, t1)
		}
	} else if t2 < t3 {
		return MIN16Int(t1, t3)
	} else {
		return MIN16Int(t2, t4)
	}
}
...
func dynalloc_analysisbak(bandLogE [][]int, bandLogE2 [][]int, nbEBands int, start int, end int, C int, offsets []int, lsb_depth int, logN []int16, isTransient int, vbr int, constrained_vbr int, eBands []int16, LM int, effectiveBytes int, tot_boost_ *BoxedValueInt, lfe int, surround_dynalloc []int) int {
	...
	for i := 0; i < end; i++ {
		noise_floor[i] = int(MULT16_16(int(0.0625*float32(int(1)<<CeltConstants.DB_SHIFT)), int(logN[i]))) +
			int(0.5*float32(int(1)<<CeltConstants.DB_SHIFT)) +
			(9-lsb_depth)<<CeltConstants.DB_SHIFT -
			int(CeltTables.EMeans[i])<<6 +
			int(MULT16_16(int(0.0062*float32(int(1)<<CeltConstants.DB_SHIFT)), int((i+5)*(i+5))))
	}
	...
	if effectiveBytes > 50 && LM >= 1 && lfe == 0 {
		...
		for c := 0; c < C; c++ {
			...
			for i := 1; i < end; i++ {
				...
				f[i] = MIN16Int(f[i-1]+int(1.5*float32(int(1)<<CeltConstants.DB_SHIFT)), bandLogE2[c][i])
			}
			for i := last - 1; i >= 0; i-- {
				f[i] = MIN16Int(f[i], MIN16Int(f[i+1]+int(2.0*float32(int(1)<<CeltConstants.DB_SHIFT)), bandLogE2[c][i]))
			}
...
func dynalloc_analysis(bandLogE [][]int, bandLogE2 [][]int, nbEBands int, start int, end int, C int, offsets []int, lsb_depth int, logN []int16, isTransient int, vbr int, constrained_vbr int, eBands []int16, LM int, effectiveBytes int, tot_boost_ *BoxedValueInt, lfe int, surround_dynalloc []int) int {
	...
	var tot_boost = 0
	var maxDepth int
	follower := InitTwoDimensionalArrayInt(2, nbEBands)
	noise_floor := make([]int, C*nbEBands) // opt: partitioned array

	MemSetLen(offsets, 0, nbEBands)
	/* Dynamic allocation code */
	maxDepth = int(0 - (int16(0.5 + (31.9)*float64((int32(1))<<(CeltConstants.DB_SHIFT)))))
	for i = 0; i < end; i++ {
		/* Noise floor must take into account eMeans, the depth, the width of the bands
		   and the preemphasis filter (approx. square of bark band ID) */
		noise_floor[i] = MULT16_16Short((int16(0.5+(0.0625)*float64((int32(1))<<(CeltConstants.DB_SHIFT)))), logN[i]) +
			...
			MULT16_16Short((int16(0.5+(0.0062)*float64((int32(1))<<(CeltConstants.DB_SHIFT)))), int16((i+5)*(i+5)))
	}
	c = 0
	for {
		for i = 0; i < end; i++ {
			maxDepth = MAX16Int(maxDepth, (bandLogE[c][i] - noise_floor[i]))
		}
		c++
		if c < C {
			continue
	...
	if effectiveBytes > 50 && LM >= 1 && lfe == 0 {
		...
		for {
			...
			for i = 1; i < end; i++ {
				...
				f[i] = MIN16Int((f[i-1] + int(int16(0.5+(1.5)*float64((int32(1))<<(CeltConstants.DB_SHIFT))))), bandLogE2[c][i])
			}
			for i = last - 1; i >= 0; i-- {
				f[i] = MIN16Int(f[i], MIN16Int((f[i+1]+int(int16(0.5+(2.0)*float64((int32(1))<<(CeltConstants.DB_SHIFT))))), bandLogE2[c][i]))
			}
			...
			for i = 2; i < end-2; i++ {
				f[i] = MAX16Int(f[i], median_of_5(bandLogE2[c], i-2)-offset)
			}
			tmp = median_of_3(bandLogE2[c], 0) - offset
			f[0] = MAX16Int(f[0], tmp)
			f[1] = MAX16Int(f[1], tmp)
			tmp = median_of_3(bandLogE2[c], end-3) - offset
			f[end-2] = MAX16Int(f[end-2], tmp)
			f[end-1] = MAX16Int(f[end-1], tmp)

			for i = 0; i < end; i++ {
				f[i] = MAX16Int(f[i], noise_floor[i])
			}
			c++
			if c < C {
				continue
		...
		if C == 2 {
			for i = start; i < end; i++ {
				...
				follower[1][i] = MAX16Int(follower[1][i], follower[0][i]-int(int16(0.5+(4.0)*float64((int32(1))<<(CeltConstants.DB_SHIFT)))))
				follower[0][i] = MAX16Int(follower[0][i], follower[1][i]-int(int16(0.5+(4.0)*float64((int32(1))<<(CeltConstants.DB_SHIFT)))))
				follower[0][i] = HALF16Int(MAX16Int(0, bandLogE[0][i]-follower[0][i]) + MAX16Int(0, bandLogE[1][i]-follower[1][i]))
			}
		} else {
			for i = start; i < end; i++ {
				follower[0][i] = MAX16Int(0, bandLogE[0][i]-follower[0][i])
		...
		for i = start; i < end; i++ {
			follower[0][i] = MAX16Int(follower[0][i], surround_dynalloc[i])
		}
		...
		for i = start; i < end; i++ {
			...
			follower[0][i] = MIN16Int(follower[0][i], int(int16(0.5+(4)*float64((int32(1))<<(CeltConstants.DB_SHIFT)))))

			width = C * int(eBands[i+1]-eBands[i]) << LM
			if width < 6 {
				boost = int(SHR32((follower[0][i]), CeltConstants.DB_SHIFT))
				boost_bits = boost * width << BITRES
			} else if width > 48 {
				boost = SHR32((follower[0][i])*8, CeltConstants.DB_SHIFT)
				boost_bits = (boost * width << BITRES) / 8
			} else {
				boost = SHR32((follower[0][i])*width/6, CeltConstants.DB_SHIFT)
				boost_bits = boost * 6 << BITRES
			}
...
func comb_filter_const(y []int, y_ptr int, x []int, x_ptr int, T int, N int, g10 int, g11 int, g12 int) {
	...
	for i = 0; i < N; i++ {
		...
		y[y_ptr+i] = x[x_ptr+i] +
			...
			MULT16_32_Q15Int(g11, ADD32(x1, x3)) +
			MULT16_32_Q15Int(g12, ADD32(x0, x4))
		x4 = x3
		x3 = x2
		x2 = x1
		x1 = x0
...
func comb_filter(y []int, y_ptr int, x []int, x_ptr int, T0 int, T1 int, N int, g0 int, g1 int, tapset0 int, tapset1 int, window []int, overlap int) {
	...
	g00 = MULT16_16_P15Int(g0, int(gains[tapset0][0]))
	g01 = MULT16_16_P15Int(g0, int(gains[tapset0][1]))
	g02 = MULT16_16_P15Int(g0, int(gains[tapset0][2]))
	g10 = MULT16_16_P15Int(g1, int(gains[tapset1][0]))
	g11 = MULT16_16_P15Int(g1, int(gains[tapset1][1]))
	g12 = MULT16_16_P15Int(g1, int(gains[tapset1][2]))
	x1 = x[x_ptr-T1+1]
	x2 = x[x_ptr-T1]
	x3 = x[x_ptr-T1-1]
	x4 = x[x_ptr-T1-2]
	...
	for i = 0; i < overlap; i++ {
		var f int
		x0 = x[x_ptr+i-T1+2]
		f = MULT16_16_Q15Int(window[i], window[i])
		y[y_ptr+i] = x[x_ptr+i] +
			MULT16_32_Q15(MULT16_16_Q15(int16(CeltConstants.Q15ONE-f), int16(g00)), x[x_ptr+i-T0]) +
			MULT16_32_Q15(MULT16_16_Q15(int16(CeltConstants.Q15ONE-f), int16(g01)), ADD32(x[x_ptr+i-T0+1], x[x_ptr+i-T0-1])) +
			MULT16_32_Q15(MULT16_16_Q15(int16(CeltConstants.Q15ONE-f), int16(g02)), ADD32(x[x_ptr+i-T0+2], x[x_ptr+i-T0-2])) +
			MULT16_32_Q15Int(MULT16_16_Q15Int(f, g10), x2) +
			MULT16_32_Q15Int(MULT16_16_Q15Int(f, g11), ADD32(x1, x3)) +
			+MULT16_32_Q15Int(MULT16_16_Q15Int(f, g12), ADD32(x0, x4))
		x4 = x3
		x3 = x2
		x2 = x1
		x1 = x0
