...
func silk_find_LTP(b_Q14 []int16, WLTP []int, LTPredCodGain_Q7 *comm.BoxedValueInt, r_lpc []int16, lag []int, Wght_Q15 []int, subfr_length int, nb_subfr int, mem_offset int, corr_rshifts []int) {
	var i, k, lshift int
	var r_ptr int
	var lag_ptr int
	var b_Q14_ptr int
	...
	for k = 0; k < nb_subfr; k++ {
		...
		boxed_rr := &comm.BoxedValueInt{0}
		boxed_rr_shift := &comm.BoxedValueInt{0}
		...
		boxed_shifts := &comm.BoxedValueInt{corr_rshifts[k]}
		...
		regu = 1
		regu = inlines.Silk_SMLAWB(regu, rr[k], int(float64(TuningParameters.LTP_DAMPING/3)*float64(int64(1)<<(16))+0.5))
		regu = inlines.Silk_SMLAWB(regu, inlines.MatrixGetPtr(WLTP, WLTP_ptr, 0, 0, SilkConstants.LTP_ORDER), (int(float64(TuningParameters.LTP_DAMPING/3)*float64(int64(1)<<(16)) + 0.5)))
		regu = inlines.Silk_SMLAWB(regu, inlines.MatrixGetPtr(WLTP, WLTP_ptr, SilkConstants.LTP_ORDER-1, SilkConstants.LTP_ORDER-1, SilkConstants.LTP_ORDER), (int(float64(TuningParameters.LTP_DAMPING/3)*float64(int64(1)<<(16)) + 0.5)))
		...
		inlines.OpusAssert((Wght_Q15[k] << 16) < math.MaxInt32)
		...
		w[k] = inlines.MatrixGetPtr(WLTP, WLTP_ptr, SilkConstants.LTP_ORDER/2, SilkConstants.LTP_ORDER/2, SilkConstants.LTP_ORDER)
	...
	for k = 0; k < nb_subfr; k++ {
		...
		g_Q26 = inlines.Silk_MUL(
			inlines.Silk_DIV32(
				...
				inlines.Silk_RSHIFT((int(float64(TuningParameters.LTP_SMOOTHING)*float64(int64(1)<<(26))+0.5)), 10)+temp32),
		...
		for i = 0; i < SilkConstants.LTP_ORDER; i++ {
			delta_b_Q14[i] = int(inlines.Silk_max_16(b_Q14[b_Q14_ptr+i], 1638))
			/* 1638_Q14 = 0.1_Q0 */
		...
		/* Q14 . Q12 */
		for i = 0; i < SilkConstants.LTP_ORDER; i++ {
			b_Q14[b_Q14_ptr+i] = int16(inlines.Silk_LIMIT_32(int(b_Q14[b_Q14_ptr+i])+inlines.Silk_SMULWB(inlines.Silk_LSHIFT_SAT32(temp32, 4), delta_b_Q14[i]), -16000, 28000))
		}
		b_Q14_ptr += SilkConstants.LTP_ORDER
	}
}
...
func silk_fit_LTP(LTP_coefs_Q16 []int, LTP_coefs_Q14 []int16, LTP_coefs_Q14_ptr int) {
	for i := 0; i < LTP_ORDER; i++ {
		val := inlines.Silk_RSHIFT_ROUND(int(LTP_coefs_Q16[i]), 2)
		if val < -32768 {
			val = -32768
		} else if val > 32767 {
			val = 32767
