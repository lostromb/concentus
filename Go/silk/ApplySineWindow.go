...
func silk_apply_sine_window(px_win []int16, px_win_ptr int, px []int16, px_ptr int, win_type int, length int) {
	...
	for k = 0; k < length; k += 4 {
		px_win[px_win_ptr+k] = int16(inlines.Silk_SMULWB(inlines.Silk_RSHIFT(S0_Q16+S1_Q16, 1), int(px[px_ptr+k])))
		px_win[px_win_ptr+k+1] = int16(inlines.Silk_SMULWB(S1_Q16, int(px[px_ptr+k+1])))
		S0_Q16 = inlines.Silk_SMULWB(S1_Q16, c_Q16) + inlines.Silk_LSHIFT(S1_Q16, 1) - S0_Q16 + 1
		S0_Q16 = inlines.Silk_min(S0_Q16, 1<<16)

		px_win[px_win_ptr+k+2] = int16(inlines.Silk_SMULWB(inlines.Silk_RSHIFT(S0_Q16+S1_Q16, 1), int(px[px_ptr+k+2])))
		px_win[px_win_ptr+k+3] = int16(inlines.Silk_SMULWB(S0_Q16, int(px[px_ptr+k+3])))
