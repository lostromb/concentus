...
func silk_CNG_exc(
	...
	for i := exc_Q10_ptr; i < exc_Q10_ptr+length; i++ {
		...
		inlines.OpusAssert(idx <= CNG_BUF_MASK_MAX)
		exc_Q10[i] = int(inlines.Silk_SAT16(inlines.Silk_SMULWW(int(exc_buf_Q14[idx]), Gain_Q16>>4)))
	}
...
func silk_CNG_Reset(psDec *SilkChannelDecoder) {
	NLSF_step_Q15 := inlines.Silk_DIV32_16(MaxInt16, psDec.LPC_order+1)
	NLSF_acc_Q15 := 0
...
func silk_CNG(
	...
	if psDec.lossCnt != 0 {
		...
		gain_Q16 := inlines.Silk_SMULWW(int(psDec.sPLC.randScale_Q14), psDec.sPLC.prevGain_Q16[1])
		...
		boxed_rand_seed := comm.BoxedValueInt{psCNG.rand_seed}
		...
		for i := 0; i < length; i++ {
			...
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-1], int(A_Q12[0]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-2], int(A_Q12[1]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-3], int(A_Q12[2]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-4], int(A_Q12[3]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-5], int(A_Q12[4]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-6], int(A_Q12[5]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-7], int(A_Q12[6]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-8], int(A_Q12[7]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-9], int(A_Q12[8]))
			sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-10], int(A_Q12[9]))

			if psDec.LPC_order == 16 {
				sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-11], int(A_Q12[10]))
				sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-12], int(A_Q12[11]))
				sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-13], int(A_Q12[12]))
				sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-14], int(A_Q12[13]))
				sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-15], int(A_Q12[14]))
				sum_Q6 = inlines.Silk_SMLAWB(sum_Q6, CNG_sig_Q10[lpci-16], int(A_Q12[15]))
			}

			CNG_sig_Q10[lpci] = inlines.Silk_ADD_LSHIFT(CNG_sig_Q10[lpci], sum_Q6, 4)
			frame[frame_ptr+i] = inlines.Silk_ADD_SAT16(frame[frame_ptr+i], int16(inlines.Silk_RSHIFT_ROUND(CNG_sig_Q10[lpci], 10)))
		}
