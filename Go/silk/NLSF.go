...
func silk_NLSF_VQ(err_Q26 []int, in_Q15 []int16, pCB_Q8 []int16, K int, LPC_order int) {
	...
	inlines.OpusAssert(err_Q26 != nil)
	...
	for i := 0; i < K; i++ {
		...
		for m := 0; m < LPC_order; m += 2 {
			diff_Q15 = inlines.Silk_SUB_LSHIFT32(int(in_Q15[m]), int(pCB_Q8[pCB_idx]), 7)
			sum_error_Q30 = inlines.Silk_SMULBB(diff_Q15, diff_Q15)
			diff_Q15 = inlines.Silk_SUB_LSHIFT32(int(in_Q15[m+1]), int(pCB_Q8[pCB_idx+1]), 7)
...
func silk_NLSF_VQ_weights_laroia(pNLSFW_Q_OUT []int16, pNLSF_Q15 []int16, D int) {
	...
	inlines.OpusAssert(pNLSFW_Q_OUT != nil)
	inlines.OpusAssert(D > 0)
	inlines.OpusAssert((D & 1) == 0)

	// First value
	tmp1_int = inlines.Silk_max_int(int(pNLSF_Q15[0]), 1)
	tmp1_int = inlines.Silk_DIV32(int(int32(1)<<(15+SilkConstants.NLSF_W_Q)), tmp1_int)
	tmp2_int = inlines.Silk_max_int(int(pNLSF_Q15[1]-pNLSF_Q15[0]), 1)
	tmp2_int = inlines.Silk_DIV32(int(int32(1)<<(15+SilkConstants.NLSF_W_Q)), tmp2_int)
	pNLSFW_Q_OUT[0] = int16(inlines.Silk_min_int(tmp1_int+tmp2_int, math.MaxInt16))

	inlines.OpusAssert(pNLSFW_Q_OUT[0] > 0)

	// Main loop
	for k = 1; k < D-1; k += 2 {
		tmp1_int = inlines.Silk_max_int(int(pNLSF_Q15[k+1]-pNLSF_Q15[k]), 1)
		tmp1_int = inlines.Silk_DIV32(int(int32(1)<<(15+SilkConstants.NLSF_W_Q)), tmp1_int)
		pNLSFW_Q_OUT[k] = int16(inlines.Silk_min_int(tmp1_int+tmp2_int, math.MaxInt16))
		inlines.OpusAssert(pNLSFW_Q_OUT[k] > 0)

		tmp2_int = inlines.Silk_max_int(int(pNLSF_Q15[k+2]-pNLSF_Q15[k+1]), 1)
		tmp2_int = inlines.Silk_DIV32(int(int32(1)<<(15+SilkConstants.NLSF_W_Q)), tmp2_int)
		pNLSFW_Q_OUT[k+1] = int16(inlines.Silk_min_int(tmp1_int+tmp2_int, math.MaxInt16))
		inlines.OpusAssert(pNLSFW_Q_OUT[k+1] > 0)
	}

	// Last value
	tmp1_int = inlines.Silk_max_int(int(int32(1)<<15-int32(pNLSF_Q15[D-1])), 1)
	tmp1_int = inlines.Silk_DIV32(int(int32(1)<<(15+SilkConstants.NLSF_W_Q)), tmp1_int)
	pNLSFW_Q_OUT[D-1] = int16(inlines.Silk_min_int(tmp1_int+tmp2_int, math.MaxInt16))
...
func silk_NLSF_residual_dequant(x_Q10 []int16, indices []int8, indices_ptr int, pred_coef_Q8 []int16, quant_step_size_Q16 int, order int16) {
	...
	for i := int(order) - 1; i >= 0; i-- {
		pred_Q10 = inlines.Silk_RSHIFT(int(inlines.Silk_SMULBB(out_Q10, int(pred_coef_Q8[i]))), 8)
		...
		out_Q10 = inlines.Silk_SMLAWB(pred_Q10, out_Q10, quant_step_size_Q16)
...
func silk_NLSF_unpack(ec_ix []int16, pred_Q8 []int16, psNLSF_CB *NLSFCodebook, CB1_index int) {
	...
	for i := 0; i < int(psNLSF_CB.order); i += 2 {
		...
		ec_sel_ptr++
		ec_ix[i] = int16(inlines.Silk_SMULBB(int((entry>>1)&7), int(2*SilkConstants.NLSF_QUANT_MAX_AMPLITUDE+1)))
		pred_Q8[i] = psNLSF_CB.pred_Q8[i+(int(entry)&1)*(int(psNLSF_CB.order)-1)]
		ec_ix[i+1] = int16(inlines.Silk_SMULBB(int((entry>>5)&7), int(2*SilkConstants.NLSF_QUANT_MAX_AMPLITUDE+1)))
		pred_Q8[i+1] = psNLSF_CB.pred_Q8[i+(inlines.Silk_RSHIFT(int(entry), 4)&1)*(int(psNLSF_CB.order)-1)+1]
	}
}
...
func silk_NLSF_stabilize(NLSF_Q15 []int16, NDeltaMin_Q15 []int16, L int) {
	...
	inlines.OpusAssert(int(NDeltaMin_Q15[L]) >= 1)
	...
	for loops = 0; loops < MAX_STABILIZE_LOOPS; loops++ {
		...
		if I == 0 {
			...
			min_center_Q15 += int(inlines.Silk_RSHIFT(int(NDeltaMin_Q15[I]), 1))

			max_center_Q15 = 1 << 15
			for k = L; k > I; k-- {
				max_center_Q15 -= int(NDeltaMin_Q15[k])
			}
			max_center_Q15 -= int(inlines.Silk_RSHIFT(int(NDeltaMin_Q15[I]), 1))

			center_freq_Q15 = int16(inlines.Silk_LIMIT_32(
				inlines.Silk_RSHIFT_ROUND(int(NLSF_Q15[I-1])+int(NLSF_Q15[I]), 1),
				min_center_Q15, max_center_Q15))
			NLSF_Q15[I-1] = center_freq_Q15 - int16(inlines.Silk_RSHIFT(int(NDeltaMin_Q15[I]), 1))
	...
	if loops == MAX_STABILIZE_LOOPS {
		...
		NLSF_Q15[0] = int16(inlines.Silk_max_int(int(NLSF_Q15[0]), int(NDeltaMin_Q15[0])))
		for i := 1; i < L; i++ {
			NLSF_Q15[i] = int16(inlines.Silk_max_int(int(NLSF_Q15[i]), int(NLSF_Q15[i-1])+int(NDeltaMin_Q15[i])))
		}
		NLSF_Q15[L-1] = int16(inlines.Silk_min_int(int(NLSF_Q15[L-1]), (1<<15)-int(NDeltaMin_Q15[L])))
		for i := L - 2; i >= 0; i-- {
			NLSF_Q15[i] = int16(inlines.Silk_min_int(int(NLSF_Q15[i]), int(NLSF_Q15[i+1])-int(NDeltaMin_Q15[i+1])))
		}
	}
}
...
func silk_NLSF_decode(pNLSF_Q15 []int16, NLSFIndices []int8, psNLSF_CB *NLSFCodebook) {
	...
	for i := 0; i < int(psNLSF_CB.order); i++ {
		W_tmp_Q9 = inlines.Silk_SQRT_APPROX(int(W_tmp_QW[i]) << (18 - SilkConstants.NLSF_W_Q))
		NLSF_Q15_tmp = int(pNLSF_Q15[i]) + inlines.Silk_DIV32_16(int(res_Q10[i])<<14, int(W_tmp_Q9))
		pNLSF_Q15[i] = int16(inlines.Silk_LIMIT(NLSF_Q15_tmp, 0, 32767))
	}
...
func silk_NLSF_del_dec_quant(indices []int8, x_Q10 []int16, w_Q5 []int16, pred_coef_Q8 []int16, ec_ix []int16, ec_rates_Q5 []int16, quant_step_size_Q16 int, inv_quant_step_size_Q6 int16, mu_Q20 int, order int16) int {
	...
	for i = 0 - SilkConstants.NLSF_QUANT_MAX_AMPLITUDE_EXT; i <= SilkConstants.NLSF_QUANT_MAX_AMPLITUDE_EXT-1; i++ {
		...
		out1_Q10 = int(inlines.Silk_ADD16(int16(out0_Q10), 1024))

		if i > 0 {
			out0_Q10 = int(inlines.Silk_SUB16(int16(out0_Q10), int16((float64(SilkConstants.NLSF_QUANT_LEVEL_ADJ)*float64(int64(1)<<(10)) + 0.5))))
			out1_Q10 = int(inlines.Silk_SUB16(int16(out1_Q10), int16((float64(SilkConstants.NLSF_QUANT_LEVEL_ADJ)*float64(int64(1)<<(10)) + 0.5))))
		} else if i == 0 {
			out1_Q10 = int(inlines.Silk_SUB16(int16(out1_Q10), int16((float64(SilkConstants.NLSF_QUANT_LEVEL_ADJ)*float64(int64(1)<<(10)) + 0.5))))
		} else if i == -1 {
			out0_Q10 = int(inlines.Silk_ADD16(int16(out0_Q10), int16((float64(SilkConstants.NLSF_QUANT_LEVEL_ADJ)*float64(int64(1)<<(10)) + 0.5))))
		} else {
			out0_Q10 = int(inlines.Silk_ADD16(int16(out0_Q10), int16((float64(SilkConstants.NLSF_QUANT_LEVEL_ADJ)*float64(int64(1)<<(10)) + 0.5))))
			out1_Q10 = int(inlines.Silk_ADD16(int16(out1_Q10), int16((float64(SilkConstants.NLSF_QUANT_LEVEL_ADJ)*float64(int64(1)<<(10)) + 0.5))))
		}
	...
	for i = int(order) - 1; ; i-- {
		...
		if nStates <= (SilkConstants.NLSF_QUANT_DEL_DEC_STATES >> 1) {
			...
			for j = 0; j < SilkConstants.NLSF_QUANT_DEL_DEC_STATES; j++ {
				x := inlines.Silk_RSHIFT(ind_sort[j], SilkConstants.NLSF_QUANT_DEL_DEC_STATES_LOG2)
				ind[j][i] += int8(x)
			}
		} else {
			// i == 0
	...
	for j = 0; j < int(order); j++ {
		...
		inlines.OpusAssert(int(indices[j]) >= 0-SilkConstants.NLSF_QUANT_MAX_AMPLITUDE_EXT)
		inlines.OpusAssert(int(indices[j]) <= SilkConstants.NLSF_QUANT_MAX_AMPLITUDE_EXT)
	}

	indices[0] = int8(int(indices[0]) + inlines.Silk_RSHIFT(ind_tmp, SilkConstants.NLSF_QUANT_DEL_DEC_STATES_LOG2))
...
func silk_NLSF_encode(NLSFIndices []int8, pNLSF_Q15 []int16, psNLSF_CB *NLSFCodebook, pW_QW []int16, NLSF_mu_Q20 int, nSurvivors int, signalType int) int {
	...
	tempIndices2 = arrayUtil.InitTwoDimensionalArrayByte(nSurvivors, SilkConstants.MAX_LPC_ORDER)
	...
	for s = 0; s < nSurvivors; s++ {
		...
		for i = 0; i < int(psNLSF_CB.order); i++ {
			...
			res_Q10[i] = int16(inlines.Silk_RSHIFT(inlines.Silk_SMULBB(int(res_Q15[i]), W_tmp_Q9), 14))
		}

		// Modify input weights accordingly
		for i = 0; i < int(psNLSF_CB.order); i++ {
			W_adj_Q5[i] = int16(inlines.Silk_DIV32_16(inlines.Silk_LSHIFT(int(pW_QW[i]), 5), int(W_tmp_QW[i])))
		}
...
func silk_NLSF2A_find_poly(o []int, cLSF []int, cLSF_ptr int, dd int) {
	...
	o[0] = inlines.Silk_LSHIFT(1, QA16)
	o[1] = 0 - cLSF[cLSF_ptr]
	for k = 1; k < dd; k++ {
		ftmp = cLSF[cLSF_ptr+(2*k)]
		/* QA*/
		o[k+1] = inlines.Silk_LSHIFT(o[k-1], 1) - int(inlines.Silk_RSHIFT_ROUND64(inlines.Silk_SMULL(ftmp, o[k]), QA16))
		for n = k; n > 1; n-- {
			o[n] += o[n-2] - int(inlines.Silk_RSHIFT_ROUND64(inlines.Silk_SMULL(ftmp, o[n-1]), QA16))
		}
		o[1] -= ftmp
	}
}
...
func silk_NLSF2A(a_Q12 []int16, NLSF []int16, d int) {
	...
	for i = 0; i < 10; i++ {
		...
		maxabs = int(inlines.Silk_RSHIFT_ROUND(int(maxabs), int(QA16+1-12)))

		if maxabs > math.MaxInt16 {
			maxabs = inlines.Silk_min_int(maxabs, 163838)
			sc_Q16 := int(math.Trunc(0.999*65536.0+0.5)) - inlines.Silk_DIV32(int(maxabs-math.MaxInt16)<<14, inlines.Silk_RSHIFT32(inlines.Silk_MUL(maxabs, int(idx+1)), 2))
	...
	if i == 10 {
		for k := 0; k < d; k++ {
			a_Q12[k] = int16(inlines.Silk_SAT16(int(inlines.Silk_RSHIFT_ROUND(int(a32_QA1[k]), int(QA16+1-12)))))
			a32_QA1[k] = int(a_Q12[k]) << (QA16 + 1 - 12)
		}
	} else {
		for k := 0; k < d; k++ {
			a_Q12[k] = int16(inlines.Silk_RSHIFT_ROUND(int(a32_QA1[k]), int(QA16+1-12)))
		}
	}
...
func silk_A2NLSF(NLSF []int16, a_Q16 []int, d int) {
	...
	for {
		...
		if (ylo <= 0 && yhi >= thr) || (ylo >= 0 && yhi <= -thr) {
			...
			NLSF[root_ix] = int16(inlines.Silk_min_32(inlines.Silk_LSHIFT(k, 8)+ffrac, math.MaxInt16))
			...
			if k > SilkConstants.LSF_COS_TAB_SZ {
				...
				if i > MAX_ITERATIONS_A2NLSF {
					...
					NLSF[0] = int16(inlines.Silk_DIV32_16(1<<15, int(d+1)))
					for k = 1; k < d; k++ {
						NLSF[k] = int16(inlines.Silk_SMULBB(k+1, int(NLSF[0])))
					}
					return
				}

				/* Error: Apply progressively more bandwidth expansion and run again */
				silk_bwexpander_32(a_Q16, d, 65536-inlines.Silk_SMULBB(10+i, i))
				/* 10_Q16 = 0.00015*/
...
func silk_process_NLSFs(psEncC *SilkChannelEncoder, PredCoef_Q12 [][]int16, pNLSF_Q15 []int16, prev_NLSFq_Q15 []int16) {
	...
	inlines.OpusAssert(NLSF_mu_Q20 <= (int(math.Trunc(0.005*float64(int64(1)<<(20)) + 0.5))))
	...
	if doInterpolate {
		...
		i_sqr_Q15 = inlines.Silk_LSHIFT(inlines.Silk_SMULBB(int(psEncC.indices.NLSFInterpCoef_Q2), int(psEncC.indices.NLSFInterpCoef_Q2)), 11)

		for i = 0; i < psEncC.predictLPCOrder; i++ {
			pNLSFW_QW[i] = int16(inlines.Silk_SMLAWB(inlines.Silk_RSHIFT(int(pNLSFW_QW[i]), 1), int(pNLSFW0_temp_QW[i]), i_sqr_Q15))
