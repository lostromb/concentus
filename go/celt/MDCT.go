...
func Clt_mdct_forward(l *MDCTLookup, input []int, input_ptr int, output []int, output_ptr int, window []int, overlap int, shift int, stride int) {
...

	{
		...
		for i = 0; i < ((overlap + 3) >> 2); i++ {
			f[yp] = inlines.MULT16_32_Q15Int(window[wp2], input[xp1+N2]) + inlines.MULT16_32_Q15Int(window[wp1], input[xp2])
			yp++
			f[yp] = inlines.MULT16_32_Q15Int(window[wp1], input[xp1]) - inlines.MULT16_32_Q15Int(window[wp2], input[xp2-N2])
			yp++
			xp1 += 2
			xp2 -= 2
			wp1 += 2
		...
		for ; i < N4; i++ {
			f[yp] = inlines.MULT16_32_Q15Int(window[wp2], input[xp2]) - inlines.MULT16_32_Q15Int(window[wp1], input[xp1-N2])
			yp++
			f[yp] = inlines.MULT16_32_Q15Int(window[wp2], input[xp1]) + inlines.MULT16_32_Q15Int(window[wp1], input[xp2+N2])
			yp++
			xp1 += 2
			xp2 -= 2
			wp1 += 2
	...
	}
	{
		...
		for i = 0; i < N4; i++ {
			...
			f2[idx] = inlines.PSHR32(inlines.MULT16_32_Q16Int(scale, yr), scale_shift)
			f2[idx+1] = inlines.PSHR32(inlines.MULT16_32_Q16Int(scale, yi), scale_shift)
		}
	}
...
func clt_mdct_backward(l *MDCTLookup, input []int, input_ptr int, output []int, output_ptr int, window []int, overlap int, shift int, stride int) {
	...
	for i = 0; i < overlap/2; i++ {
		x1 := output[xp1]
		x2 := output[yp1]
		output[yp1] = inlines.MULT16_32_Q15Int(window[wp2], x2) - inlines.MULT16_32_Q15Int(window[wp1], x1)
		yp1++
		output[xp1] = inlines.MULT16_32_Q15Int(window[wp1], x2) + inlines.MULT16_32_Q15Int(window[wp2], x1)
		xp1--
		wp1++
		wp2--
	}
