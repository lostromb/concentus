...
func interp_bits2pulses(m *CeltMode, start int, end int, skip_start int, bits1 []int, bits2 []int, thresh []int, cap []int, total int, _balance *comm.BoxedValueInt, skip_rsv int, intensity *comm.BoxedValueInt, intensity_rsv int, dual_stereo *comm.BoxedValueInt, dual_stereo_rsv int, bits []int, ebits []int, fine_priority []int, C int, LM int, ec *comm.EntropyCoder, encode int, prev int, signalBandwidth int) int {
	...
	for {
		...
		left = total - psum
		percoeff = inlines.Celt_udiv(left, int(m.eBands[codedBands]-m.eBands[start]))
		left -= int(m.eBands[codedBands]-m.eBands[start]) * percoeff
		rem = inlines.IMAX(left-int(m.eBands[j]-m.eBands[start]), 0)
		band_width = int(m.eBands[codedBands] - m.eBands[j])
		band_bits = bits[j] + percoeff*band_width + rem
		if band_bits >= inlines.IMAX(thresh[j], alloc_floor+(1<<BITRES)) {
			if encode != 0 {
	...
	for j = start; j < codedBands; j++ {
		tmp := inlines.IMIN(left, int(m.eBands[j+1]-m.eBands[j]))
		bits[j] += tmp
		left -= tmp
	}
	...
	for j = start; j < codedBands; j++ {
		...
		if N > 1 {
			...
			ebits[j] = inlines.IMAX(0, bits[j]+offset+(den<<(BITRES-1)))
			ebits[j] = inlines.Celt_udiv(ebits[j], den) >> BITRES
			...
			excess = inlines.MAX32(0, bit-(C<<BITRES))
			bits[j] = bit - excess
			ebits[j] = 0
			fine_priority[j] = 1
		}
		...
		if excess > 0 {
			extra_fine := inlines.IMIN(excess>>(stereo+BITRES), CeltConstants.MAX_FINE_BITS-ebits[j])
			ebits[j] += extra_fine
	...
	for ; j < end; j++ {
		ebits[j] = bits[j] >> stereo >> BITRES
		inlines.OpusAssert(C*ebits[j]<<BITRES == bits[j])
		bits[j] = 0
		if ebits[j] < 1 {
			fine_priority[j] = 1
		} else {
...
func compute_allocation(m *CeltMode, start int, end int, offsets []int, cap []int, alloc_trim int, intensity *comm.BoxedValueInt, dual_stereo *comm.BoxedValueInt, total int, balance *comm.BoxedValueInt, pulses []int, ebits []int, fine_priority []int, C int, LM int, ec *comm.EntropyCoder, encode int, prev int, signalBandwidth int) int {
	...
	for j := start; j < end; j++ {
		thresh[j] = inlines.IMAX(C<<BITRES, int(3*(m.eBands[j+1]-m.eBands[j])<<LM<<BITRES)>>4)
