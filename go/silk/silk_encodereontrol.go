...
func NewSilkEncoderControl() *SilkEncoderControl {
	...
	s.PredCoef_Q12 = arrayUtil.InitTwoDimensionalArrayShort(2, SilkConstants.MAX_LPC_ORDER)
	s.pitchL = make([]int, SilkConstants.MAX_NB_SUBFR)
	return s
}

func (s *SilkEncoderControl) Reset() {
	arrayUtil.MemSetLen(s.Gains_Q16, 0, SilkConstants.MAX_NB_SUBFR)
	arrayUtil.MemSetLen(s.PredCoef_Q12[0], 0, SilkConstants.MAX_LPC_ORDER)
	arrayUtil.MemSetLen(s.PredCoef_Q12[1], 0, SilkConstants.MAX_LPC_ORDER)
	arrayUtil.MemSetLen(s.LTPCoef_Q14, 0, SilkConstants.LTP_ORDER*SilkConstants.MAX_NB_SUBFR)
	s.LTP_scale_Q14 = 0
	arrayUtil.MemSetLen(s.pitchL, 0, SilkConstants.MAX_NB_SUBFR)
	...
	arrayUtil.MemSetLen(s.LF_shp_Q14, 0, SilkConstants.MAX_NB_SUBFR)
	arrayUtil.MemSetLen(s.GainsPre_Q14, 0, SilkConstants.MAX_NB_SUBFR)
	arrayUtil.MemSetLen(s.HarmBoost_Q14, 0, SilkConstants.MAX_NB_SUBFR)
	arrayUtil.MemSetLen(s.Tilt_Q14, 0, SilkConstants.MAX_NB_SUBFR)
	arrayUtil.MemSetLen(s.HarmShapeGain_Q14, 0, SilkConstants.MAX_NB_SUBFR)
	s.Lambda_Q10 = 0
