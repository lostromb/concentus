...
func silk_LDL_factorize(A []int, A_ptr int, M int, L_Q16 []int, inv_D []int) {
	...
	for loop_count = 0; loop_count < M && status == 1; loop_count++ {
		...
		for j = 0; j < M; j++ {
			...
			tmp_32 = inlines.Silk_SUB32(inlines.MatrixGetPtr(A, A_ptr, j, j, M), tmp_32)

			if tmp_32 < diag_min_value {
				tmp_32 = inlines.Silk_SUB32(inlines.Silk_SMULBB(loop_count+1, diag_min_value), tmp_32)
				for i = 0; i < M; i++ {
					inlines.MatrixSet(A, A_ptr, i, i, M, inlines.Silk_ADD32(inlines.MatrixGetPtr(A, A_ptr, i, i, M), tmp_32))
				}
				status = 1
				break
			}
			...
			inlines.MatrixSet5(L_Q16, j, j, M, 65536)
			scratch1 = A
			...
			for i = j + 1; i < M; i++ {
				...
				inlines.MatrixSet5(L_Q16, i, j, M, inlines.Silk_ADD32(
