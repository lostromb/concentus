...
func hp_cutoff(input []int16, input_ptr int, cutoff_Hz int, output []int16, output_ptr int, hp_mem []int, len int, channels int, Fs int) {
	...
	inlines.OpusAssert(cutoff_Hz <= int(math.MaxInt32)/int(math.Trunc((1.5*3.14159/1000)*(1<<(19))+0.5)))
	Fc_Q19 = inlines.Silk_DIV32_16(inlines.Silk_SMULBB(int(math.Trunc((1.5*3.14159/1000)*(1<<(19))+0.5)), cutoff_Hz), Fs/1000)
	inlines.OpusAssert(Fc_Q19 > 0 && Fc_Q19 < 32768)

	r_Q28 = int(math.Trunc((1.0)*(1<<(28))+0.5)) - inlines.Silk_MUL(int(math.Trunc((0.92)*(1<<(9))+0.5)), Fc_Q19)
	...
	A_Q28[0] = inlines.Silk_SMULWW(r_Q22, inlines.Silk_SMULWW(Fc_Q19, Fc_Q19)-int(math.Trunc((2.0)*(1<<22)+0.5)))
	A_Q28[1] = inlines.Silk_SMULWW(r_Q22, r_Q22)

	silk.Silk_biquad_alt_ptr(input, input_ptr, B_Q28, A_Q28, hp_mem, 0, output, output_ptr, len, channels)
	if channels == 2 {
		silk.Silk_biquad_alt_ptr(input, input_ptr+1, B_Q28, A_Q28, hp_mem, 2, output, output_ptr+1, len, channels)
	}
}
...
func dc_reject(input []int16, input_ptr int, cutoff_Hz int, output []int16, output_ptr int, hp_mem []int, len int, channels int, Fs int) {
	...
	for c = 0; c < channels; c++ {
		for i = 0; i < len; i++ {
			var x, tmp, y int32
			x = int32(inlines.SHL32(inlines.EXTEND32(input[channels*i+c+input_ptr]), 15))
			/* First stage */
			tmp = x - int32(hp_mem[2*c])
			hp_mem[2*c] = hp_mem[2*c] + inlines.PSHR32(int(x-int32(hp_mem[2*c])), shift)
			/* Second stage */
			y = tmp - int32(hp_mem[2*c+1])
			hp_mem[2*c+1] = hp_mem[2*c+1] + inlines.PSHR32(int(tmp-int32(hp_mem[2*c+1])), shift)
			output[channels*i+c+output_ptr] = inlines.EXTRACT16(int(int32(inlines.SATURATE(inlines.PSHR32(int(y), 15), 32767))))
		}
	}
}
...
func stereo_fade(pcm_buf []int16, g1 int, g2 int, overlap48 int, frame_size int, channels int, window []int, Fs int) {
	...
	for i := 0; i < overlap; i++ {
		var diff int
		var g, w int
		w = inlines.MULT16_16_Q15Int(window[i*inc], window[i*inc])
		g = inlines.SHR32(inlines.MAC16_16IntAll(inlines.MULT16_16(w, int(g2)), CeltConstants.Q15ONE-w, int(g1)), 15)
		diff = int(inlines.EXTRACT16(inlines.HALF32(int(pcm_buf[i*channels]) - int(pcm_buf[i*channels+1]))))
		diff = inlines.MULT16_16_Q15Int(int(g), diff)
	...
	for i := overlap; i < frame_size; i++ {
		var diff int
		diff = int(inlines.EXTRACT16(inlines.HALF32(int(pcm_buf[i*channels]) - int(pcm_buf[i*channels+1]))))
		diff = inlines.MULT16_16_Q15Int(int(g2), diff)
		pcm_buf[i*channels] = int16(int(pcm_buf[i*channels]) - diff)
		pcm_buf[i*channels+1] = int16(int(pcm_buf[i*channels+1]) + diff)
	}
}
...
func gain_fade(buffer []int16, buf_ptr int, g1 int, g2 int, overlap48 int, frame_size int, channels int, window []int, Fs int) {
	...
	if channels == 1 {
		for i := 0; i < overlap; i++ {
			...
			w = inlines.MULT16_16_Q15Int(window[i*inc], window[i*inc])
			g = inlines.SHR32(inlines.MAC16_16IntAll(inlines.MULT16_16(w, int(g2)), CeltConstants.Q15ONE-w, int(g1)), 15)
			buffer[buf_ptr+i] = int16(inlines.MULT16_16_Q15Int(int(g), int(buffer[buf_ptr+i])))
		}
	} else {
		for i := 0; i < overlap; i++ {
			var g, w int
			w = inlines.MULT16_16_Q15Int(window[i*inc], window[i*inc])
			g = inlines.SHR32(inlines.MAC16_16IntAll(inlines.MULT16_16(w, int(g2)), CeltConstants.Q15ONE-w, int(g1)), 15)
			buffer[buf_ptr+i*2] = int16(inlines.MULT16_16_Q15Int(int(g), int(buffer[buf_ptr+i*2])))
			buffer[buf_ptr+i*2+1] = int16(inlines.MULT16_16_Q15Int(int(g), int(buffer[buf_ptr+i*2+1])))
		}
	}
	for c := 0; c < channels; c++ {
		for i := overlap; i < frame_size; i++ {
			buffer[buf_ptr+i*channels+c] = int16(inlines.MULT16_16_Q15Int(int(g2), int(buffer[buf_ptr+i*channels+c])))
		}
	}
}
...
func transient_boost(E []float32, E_ptr int, E_1 []float32, LM int, maxM int) float32 {
	...
	return inlines.MIN16Float(1, float32(math.Sqrt(float64(inlines.MAX16Float(0, .05*(metric-2))))))
}
...
func compute_stereo_width(pcm []int16, pcm_ptr int, frame_size int, Fs int, mem *StereoWidthState) int {
	...
	mem.XX += inlines.MULT16_32_Q15Int(int(short_alpha), xx-mem.XX)
	mem.XY += inlines.MULT16_32_Q15Int(int(short_alpha), xy-mem.XY)
	mem.YY += inlines.MULT16_32_Q15Int(int(short_alpha), yy-mem.YY)
	mem.XX = inlines.MAX32(0, mem.XX)
	mem.XY = inlines.MAX32(0, mem.XY)
	mem.YY = inlines.MAX32(0, mem.YY)
	if inlines.MAX32(mem.XX, mem.YY) > int(math.Trunc(0.5+(8e-4)*(1<<18))) {
		...
		width := inlines.MULT16_16_Q15Int(inlines.Celt_sqrt(1<<30-inlines.MULT16_16(corr, corr)), ldiff)
		mem.smoothed_width += (width - mem.smoothed_width) / int(frame_rate)
		mem.max_follower = inlines.MAX16Int(mem.max_follower-int(math.Trunc(0.5+(0.02)*(1<<15)))/int(frame_rate), mem.smoothed_width)
	} else {
		mem.smoothed_width = 0
		mem.max_follower = 0
	}
	return inlines.EXTEND32Int(inlines.MIN32(CeltConstants.Q15ONE, 20*mem.max_follower))
}
...
func smooth_fade(in1 []int16, in1_ptr int, in2 []int16, in2_ptr int, output []int16, output_ptr int, overlap int, channels int, window []int, Fs int) {
	...
	for c := 0; c < channels; c++ {
		for i := 0; i < overlap; i++ {
			w := inlines.MULT16_16_Q15Int(window[i*inc], window[i*inc])
			output[output_ptr+(i*channels)+c] = int16(inlines.SHR32(inlines.MAC16_16IntAll(inlines.MULT16_16(w, int(in2[in2_ptr+(i*channels)+c])), CeltConstants.Q15ONE-w, int(in1[in1_ptr+(i*channels)+c])), 15))
		}
	}
}
