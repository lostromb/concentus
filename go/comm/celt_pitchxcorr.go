...
func Pitch_xcorr(_x []int, _y []int, xcorr []int, len int, max_pitch int) int {
	...
	for i = 0; i < max_pitch-3; i += 4 {
		...
		sum2.Val = 0
		sum3.Val = 0
		kernels.Xcorr_kernel_int(_x, _y, i, &sum0, &sum1, &sum2, &sum3, len)
		xcorr[i] = sum0.Val
		xcorr[i+1] = sum1.Val
		xcorr[i+2] = sum2.Val
		xcorr[i+3] = sum3.Val
	...
	for ; i < max_pitch; i++ {
		inner_sum := kernels.Celt_inner_prod_int(_x, 0, _y, i, len)
		xcorr[i] = inner_sum
...
func Pitch_xcorr1(_x []int16, _x_ptr int, _y []int16, _y_ptr int, xcorr []int, len int, max_pitch int) int {
	...
	for ; i < max_pitch; i++ {
		inner_sum := kernels.Celt_inner_prod(_x, _x_ptr, _y, _y_ptr+i, len)
		xcorr[i] = inner_sum
		maxcorr = inlines.MAX32(maxcorr, inner_sum)
	}
	return maxcorr
...
func pitch_xcorr2(_x []int16, _y []int16, xcorr []int, len int, max_pitch int) int {
	...
	/* In case max_pitch isn't a multiple of 4, do non-unrolled version. */
	for ; i < max_pitch; i++ {
		inner_sum := kernels.Celt_inner_prod_short(_x, _y, i, len)
		xcorr[i] = inner_sum
		maxcorr = inlines.MAX32(maxcorr, inner_sum)
	}
	return maxcorr
